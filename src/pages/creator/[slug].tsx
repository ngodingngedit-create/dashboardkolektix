  ...
  return (
    <div className='min-h-screen mx-auto'>
      <div className="w-full">
        <img 
          src="https://storage.nu.or.id/storage/post/16_9/mid/img-20211125-094750_1637808730.jpg" 
      ...
      <div className='mx-auto w-full md:w-10/12 px-4 sm:px-6 md:px-8 lg:px-0'>
        ...
        <div className='flex flex-col md:flex-row py-10 md:justify-between'>
          <div>
            <p className='text-xl md:text-2xl font-semibold text-dark'>
              {creatorData?.name || 'Nama Organizer'}
            </p>
            <p className='text-sm md:text-base text-grey'>
              Member sejak {getMemberSinceYear()}
            </p>
          </div>
          <div className='flex flex-col md:flex-row gap-4 md:gap-6 mt-6 md:mt-0'>
            <Button
              label='Follow'
              color='primary'
              className='w-full h-10 md:w-40 text-white disabled:bg-gray-400 disabled:text-gray-400 disabled:opacity-50'
            />
            <Button
              label='Kirim Pesan'
              color='secondary'
        ...
        <div className="w-full">
          ...
          >
            {tabData.map(({ key, label }) => (
              <Tab 
                key={key} 
                title={<div className="flex items-center text-medium space-x-2"><span>{label}</span></div>} 
              />
            ))}
          </Tabs>
