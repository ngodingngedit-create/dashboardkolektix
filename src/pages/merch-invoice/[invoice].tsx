...
import { Icon } from "@iconify/react/dist/iconify.js";
import { Box, Button, Card, Container, Divider, Flex, Image, NumberFormatter, ScrollArea, SimpleGrid, Stack, Table, Text, Title } from "@mantine/core";
import _ from "lodash";
import Link from "next/link";
import { InvoiceResponse } from "./type";
import { useEffect, useMemo, useState } from "react";
...
export default function Invoice() {
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      ...
      // Karena NEXT_PUBLIC_WS_URL sudah mengandung /api/ di akhir,
      // kita perlu menyesuaikan path-nya
      const baseUrl = apiUrl.endsWith('/') ? apiUrl.slice(0, -1) : apiUrl;
      
      // Jika baseUrl sudah mengandung /api/, kita hanya perlu menambahkan path setelahnya
      let downloadUrl;
      if (baseUrl.endsWith('/api')) {
        // Contoh: https://api.kolektix.cloud/api
        downloadUrl = `${baseUrl}/order-product/download/${invoice}`;
      } else if (baseUrl.endsWith('/api/')) {
        // Contoh: https://api.kolektix.cloud/api/ (dari environment variable Anda)
        downloadUrl = `${baseUrl}order-product/download/${invoice}`;
      } else {
        // Fallback: tambahkan /api/ jika belum ada
