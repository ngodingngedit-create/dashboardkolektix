  ...
  const handleFormSubmit = async (values: typeof form.values) => {
    if (isEditMode && selectedUser) {
      ...
      const userData = {
        ...
        is_creator: values.is_creator,
        verified_status_id: values.verified_status_id,
        password: "defaultpassword123", // Atau tambahkan field password di form
        password_confirmation: "defaultpassword123",
      };

      try {
        await fetch({
          url: "users", // Sesuaikan endpoint user
          method: "POST",
          data: userData,
          before: () => setLoading.append("submit"),
          success: (response) => {
        ...
        <TableData
          ...
          mapData={(e: any) => {
            ...
            return {
              ...
              contact: (
                <Stack gap={2}>
                  {userData.has_user?.phone && <Text size="sm">{userData.has_user.phone}</Text>}
                  {userData.phone_number && (
                    <Text size="xs" c="dimmed">
                      Creator: {userData.phone_number}
                    </Text>
                  )}
                </Stack>
              ),
              creator_info: (
                <Stack gap={2}>
                  {userData.name && <Text size="sm">{userData.name}</Text>}
                  {userData.name_event_organizer && (
                    <Text size="xs" c="dimmed">
                      EO: {userData.name_event_organizer}
                    </Text>
                  )}
                  {userData.location && (
                    <Text size="xs" c="dimmed">
              ...
              creator_status: (
                <Stack gap={2}>
                  <Badge color={userData.has_user?.is_creator === 1 ? "green" : "gray"} variant="light" size="sm">
                    {userData.has_user?.is_creator === 1 ? "Creator" : "User Biasa"}
                  </Badge>
                  <Text size="xs" c="dimmed">
                    Status: {userData.status === "active" ? "Aktif" : "Nonaktif"}
                  </Text>
                </Stack>
              ),
              verification: (
