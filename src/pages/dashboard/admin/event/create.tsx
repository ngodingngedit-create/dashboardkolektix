...
import { Container, Title, Paper, Grid, TextInput, NumberInput, Switch, Textarea, Button, Group, Tabs, FileInput, Image as MantineImage, Select } from "@mantine/core";
...
export default function CreateEventAdmin() {
  ...
  const form = useForm({
    initialValues: {
      ...
      term_condition: "",
      save_as_draft: "0",
      event_status_id: 3, // Default Review
      activity_status: 1,
    ...
    validate: {
      name: (value) => (value ? null : "Nama event wajib diisi"),
      creator_id: (value) => (value ? null : "Pilih penyelenggara"),
    }
  });
  ...
  const fetchCreators = async () => {
    ...
    try {
      ...
      let creatorsData = [];
      if (Array.isArray(res.data)) creatorsData = res.data;
      else if (res.data && Array.isArray(res.data.data)) creatorsData = res.data.data;
      
      setCreators(creatorsData);
    } catch (error) {
      console.error("Gagal memuat creator", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Container size="xl" py="xl">
      ...
      <Group justify="space-between" mb="lg">
        <Title order={2}>Tambah Event Baru (Admin)</Title>
      ...
      <form onSubmit={form.onSubmit(handleSubmit)}>
              ...
              <Grid>
                <Grid.Col span={12}>
                  <Select
                    ...
                    data={creators.map(c => ({
                      value: String(c.id),
                      label: c.name || c.name_event_organizer || "Unknown"
                    }))}
                    searchable
                    required
                    {...form.getInputProps("creator_id")}
                  />
                </Grid.Col>
                <Grid.Col span={{ base: 12, md: 6 }}>
                  <TextInput label="Nama Event" placeholder="Masukkan nama event..." required {...form.getInputProps("name")} />
                </Grid.Col>
                <Grid.Col span={{ base: 12, md: 6 }}>
                ...
                <Grid.Col span={{ base: 12, md: 4 }}>
                  <NumberInput label="Status Event (3=Review, 1=Active)" {...form.getInputProps("event_status_id")} />
                </Grid.Col>
                <Grid.Col span={{ base: 12, md: 4 }}>
        ...
        <Group justify="flex-end" mt="xl">
          ...
          <Button type="submit" color="indigo">Daftarkan Event</Button>
        </Group>
      </form>
    </Container>
  );
