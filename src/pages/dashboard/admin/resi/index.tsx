...
export default function OrderTracking() {
    ...
    const getOrderDetail = async (transaction: Transaction) => {
        const orderDetailData: OrderDetail = {
            ...
            order_courier_id: 0, // Will be updated if needed
        ...
        form.setValues({
            courier_company: '', // Should fetch these from order details if available
            courier_service: '',
            resi_no: '',
            etd: '',
            etd_time: '',
    ...
    const fetchInvoiceById = async (invoiceId: string) => {
        ...
        await fetch({
            url: `order-product-invoice/${invoiceId}`,
            method: 'GET',
            before: () => setLoading.append('fetchinvoice'),
            success: (response: any) => {
                const data = response?.data;
                if (data) {
                    const orderDetailData: OrderDetail = {
                        order_id: data.id,
                        order_courier_id: data.courier?.id || 0,
                        invoice_no: data.invoice_no,
                        customer_name: data.address?.nama_penerima || '-',
                    ...
                    form.setValues({
                        ...
                        resi_no: '', // Usually we update this
                        etd: '',
                        etd_time: '',
                        delivery_id: '',
                    });
            ...
            complete: () => setLoading.filter(e => e !== 'fetchinvoice'),
        });
    };
    ...
    const submitTracking = async () => {
        ...
        const submitData = {
            resi_no: form.values.resi_no,
            courier_company: form.values.courier_company,
            courier_service: form.values.courier_service,
            etd: form.values.etd,
            etd_time: form.values.etd_time,
            delivery_id: form.values.delivery_id,
        };

        await fetch({
            url: `order-product-invoice/${orderDetail.invoice_no}`,
            method: 'POST', // Based on project pattern
            data: submitData,
                            ...
                            <Stack gap="md" mb="lg">
                                ...
                                <Stack gap="xs">
                                    ...
                                    <Select
                                        ...
                                        onChange={(value) => {
                                            if (value) {
                                                const transaction = transactions.find(t => t.invoice_no === value);
                                                if (transaction) getOrderDetail(transaction);
                                            }
                                        }}
                                    />
                                </Stack>
                                ...
                                <Stack gap="xs">
                                    ...
                                    <Group grow align="flex-end">
                                        ...
                                        <Button 
                                            size="md" 
                                            radius="md" 
                                            onClick={() => fetchInvoiceById(searchQuery)}
                            ...
                            <Stack gap="lg">
                                ...
                                <Group grow mt="xl">
                                    ...
                                    <Button
                                        ...
                                        styles={{
                                            root: {
                                                background: 'linear-gradient(45deg, #7b61ff 0%, #b456ff 100%)',
                                                border: 0
                                            }
                                        }}
