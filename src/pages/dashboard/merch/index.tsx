...
const Merch = () => {
  const [modalCreate, setModalCreate] = useState<number>();
  ...
  return (
    <div className={`p-[30px_20px] text-black flex flex-col gap-[25px]`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-[20px]">
        ...
        <button onClick={() => setModalCreate(0)} className={`text-[#0B387C] flex items-center gap-[8px] p-[10px_16px] border border-[#E2EDFF] hover:border-[#0B387C] rounded-full`}>
                  ...
                  {(splittedByStatus(e[0]) ?? []).map((e, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell className={`whitespace-nowrap`}>
                        <NumberFormatter value={parseInt(e.selling_price)} prefix="Rp " />
                      </TableCell>
                      <TableCell>-</TableCell>
                      <TableCell>
                        <div className="flex items-center gap-[10px]">
                          ...
                          <button onClick={() => setModalCreate(e.id)}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
