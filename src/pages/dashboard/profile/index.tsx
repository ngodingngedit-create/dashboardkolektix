...
const Profile = () => {
  ...
  return (
    <>
      <div className='max-w-3xl mx-auto pt-10 text-dark h-[90vh] px-4 sm:px-8 md:px-12 lg:px-0'>
        <div className='border-2 border-primary-light-200 rounded-xl p-4 flex flex-col gap-3'>
          ...
          <InputField
            ...
            fullWidth
            onChange={(e) => setForm({ ...form, no_identity: e.target.value })}
          />
          <InputField
            type='number'
            label='Nomor Telepon'
      ...
      <div className='border-t border-t-primary-light-200 sticky bottom-0 flex justify-end p-5'>
        {hasData ? (
          <Button label='Update Dokumen' color='primary' onClick={onUpdate} disabled={loading} />
        ) : (
          <Button label='Kirim Dokumen' color='primary' onClick={onSubmit} disabled={loading} />
        )}
      </div>
    </>
  );
