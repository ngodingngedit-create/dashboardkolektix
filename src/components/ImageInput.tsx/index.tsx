    ...
    return (
        <InputWrapper {...{ label, description, error }} withAsterisk={required}>
            <Box pos="relative" className={`[&_.clsBtn]:hover:!block`} py={10}>
                    <Card
                        w="100%"
                        bg={(!value || value.length === 0) ? "gray.1" : undefined}
                        withBorder
                        p={0}
                        radius={8}
                        ...
                        pos="relative"
                    >
                        <Center h="100%" pos="relative" style={{ width: "100%" }}>
                            {value && value.length > 0 ? (
                                <ScrollArea w="100%" type="auto" offsetScrollbars>
                                    <Group gap={8} wrap="nowrap" align="center">
                                        {value.map((img, idx) => (
                                            <Box key={idx} pos="relative" style={{ minWidth: dimension ? dimension[0] : 100, minHeight: dimension ? dimension[1] : 100 }}>
                                                { 
                                                    (()=>{
                                                        const infotext = floattext || "Utama";
                                                        return (
                                                            idx === 0 && (
                                                                <Text
                                                                    size="xs"
                                                                    className={`absolute top-[80px] left-[20px] bg-[#0B387C] rounded-full px-[20px] z-10 w-[60px] text-center`}
                                                                    style={{ color: "#fff" }}
                                                                >
                                                                    {infotext}
                                                                </Text>
                                                ...
                                                {(() => {
                                                    ...
                                                    const handler = {
                                                        apply(target: any, thisArg: any, argumentsList: any[]) {
                                                            return hideDelete ? null : target.apply(thisArg, argumentsList);
                                                        }
                                                    };
                                                ...
                                                <Image
                                                    ...
                                                    w={dimension ? dimension[0] : 100}
                                                    h={dimension ? dimension[1] : 100}
