...
// Custom hook untuk deteksi domain
const useDomain = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      setDomain(hostname);
      setShowAllDebug(debug === 'showall');
      
      // Production: kolektix.com (tanpa debug)
      setIsProduction(hostname === 'kolektix.com' && debug !== 'showall');
      
      // Cloud: kolektix.cloud atau localhost
      setIsCloud(hostname.includes('cloud') || hostname === 'localhost');
      
      console.log('📍 Current Domain:', hostname);
      console.log('🎯 Is Production:', hostname === 'kolektix.com' && debug !== 'showall');
      console.log('☁️ Is Cloud:', hostname.includes('cloud') || hostname === 'localhost');
      console.log('🔧 Debug Mode:', debug === 'showall');
    }
  }, []);

  return { isProduction, isCloud, domain, showAllDebug };
};
...
export default function NavbarComponent({ children }: { children: ReactNode }) {
  ...
  // Domain detection
  const { isProduction, isCloud, showAllDebug } = useDomain();

  // Conditional untuk menampilkan Talent & Venue
  const showTalentVenue = showAllDebug || !isProduction; // Muncul di semua tempat KECUALI production (kolektix.com)
