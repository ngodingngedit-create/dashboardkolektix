...
export default function CreateMerchandiseAdmin({ onClose, id, creatorId }: Readonly<ComponentProps & { creatorId?: string }>) {
  ...
  const handleSizeChartDelete = useCallback(
    (idx: number) => {
      form.setValues((prev: any) => ({
        ...prev,
        size_chart: (prev.size_chart ?? []).filter((_: any, z: number) => z !== idx),
      }));
    },
    [form]
  );
      ...
      <div className="flex flex-col w-full">
        ...
        <div className="sticky top-0 bg-white z-30 border-b border-light-grey pb-4 px-[20px] pt-[20px]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-[30px] font-[600] m-0">{!Boolean(id) ? "Buat" : "Update"} Merchandise</h2>
              <p className="text-grey m-0">{!Boolean(id) ? "Lengkapi form dibawah untuk membuat Merchandise" : "Lengkapi form dibawah untuk update Merchandise"}</p>
              ...
              {selectedCreatorData && (
                <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-lg inline-block">
                  <p className="text-sm text-blue-800">
                    <span className="font-medium">Creator:</span> {selectedCreatorData.name}
              ...
              {!id && creatorId && !selectedCreatorData && (
                <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg inline-block">
                  <p className="text-sm text-yellow-800">Memuat data creator...</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="py-[20px] px-[20px] flex flex-col gap-[30px]">
            ...
            <div className="border border-[#E2EDFF] rounded-[8px]">
              ...
              <div className="p-[24px_16px] flex flex-col gap-[20px]">
                ...
                <div className="flex flex-wrap gap-[20px]">
                  ...
                  <div className="flex-grow overflow-x-auto">
                    <InputWrapper error={form.errors.size_chart as string}>
                      <Box pb={10}>
                        <ImageInputMultiple
                ...
                <div className="flex flex-wrap items-center gap-[5px] md:gap-[20px]">
                  ...
                  <div className="flex-grow [&_*]:border-[#E2EDFF]">
                    <Flex gap={10}>
                      <ActionIcon variant="filled" bg="#0B387C" size={36} radius="md" title="Generate SKU" onClick={() => form.setFieldValue('sku', `${currentCreatorId}-${Math.floor(100000 + Math.random() * 900000)}`)}>
                        ...
                        <Table.Tr key={i} style={{ borderBottom: '1px solid #f0f0f0' }}>
                          ...
                          <Table.Td>
                            <Flex gap={5}>
                              ...
                              <ActionIcon variant="filled" bg="#0B387C" size={36} radius="md" onClick={() => form.setFieldValue(`variant.${i}.sku`, `${currentCreatorId}-${Math.floor(100000 + Math.random() * 900000)}`)} title="Generate SKU">
                                <Icon icon="ph:link-bold" className="text-lg" />
                              </ActionIcon>
                            </Flex>
                          </Table.Td>
                          ...
                          <Table.Td>
                            <Switch color="#0B387C" checked={(form.values.variant || [])[i].status || false} onChange={(e) => form.setFieldValue(`variant.${i}.status`, e.target.checked)} error={form.errors[`variant.${i}.status`]} />
                          </Table.Td>
                          <Table.Td>
                            <Button
                              variant="light"
            ...
            <div className="border border-[#E2EDFF] rounded-[8px]">
              <Flex align="center" justify="space-between" className={`p-[12px_16px] border-b border-[#E2EDFF]`}>
                ...
                <Switch defaultChecked size="md" color="#0B387C" checked={form.values.status || false} onChange={(e) => form.setValues({ status: e.target.checked })} />
              </Flex>
      ...
      >
        {activePromoVariant !== null && (form.values.variant || [])[activePromoVariant] && (
          <div className="flex flex-col gap-[20px]">
            <Switch
              color="#0B387C"
              checked={(form.values.variant || [])[activePromoVariant].is_promo || false}
