...
const MerchandiseCard = ({ name, price, sale, creator }: MerchCardProps & {}) => {
  const [bookmark, setBookmark] = useState<boolean>(false);
  return (
    <Link href='/merchandise/detail' className='min-w-44 bg-white rounded-lg border border-primary-light-200 shadow-md ml-1 md:ml-0 w-full'>
      <div className={``}>
        <Image className={`${styles.cardImg} rounded-t-lg`} src={Foto} alt='' />
      </div>
      <div>
        <div className={`p-[10px]`}>
          <p className='mb-2 text-dark font-[500] leading-[140%] text-[0.8rem]'>Rajasinga Medan Tour Tshirt | Official Merchandise</p>
          <div>
            <p className='text-dark text-[0.8rem] font-[500]'>Rp 1.125.000</p>
            <p className='text-dark-grey text-[0.7rem] line-through'>Rp 1.125.000</p>
          </div>
