      ...
      <div className='p-3'>
        ...
        <p className='mb-3 font-normal text-sm'>
          ...
          <span className='text-dark'>
            {`${formatDate(date)} ${date !== end ? '-' + formatDate(end) : ''} ${formatYear(end)}`}
          </span>
        </p>
        ...
        (event_status_id === 3 ?  (
          <div className='border-t-1.5 border-dashed border-primary-light-200 grid grid-cols-2 justify-items-center w-full p-3 gap-x-2'>
            <Button
              ...
              onClick={() => router.push(`/dashboard/my-event/${slug}`)}
            />
            <Button
              label='Check In'
              color='secondary'
              fullWidth
              className='text-xs'
              onClick={() => router.push(`/dashboard/scan/${slug}`)}
            />
            <Button
              label='Penjualan'
              color='secondary'
              fullWidth
              className='text-xs'
              onClick={() => router.push(`/dashboard/my-event/sell/${slug}`)}
            />
          </div>
        ) : event_status_id === 2 ? (
          <div className='border-t-1.5 border-dashed border-primary-light-200 grid grid-cols-2 justify-items-center w-full p-3 gap-x-2'>
            <Button
              ...
              fullWidth
              className='text-xs'
              onClick={() => router.push(`/dashboard/my-event/checkin/${slug}`)}
            />
            <Button
              label='Edit'
              color='secondary'
