...
const FirstStep = ({
  ...
  const handleInput = (index: number, field: keyof Form, value: string) => {
    ...
    const isFormValid = newForm.every((item) => {
      return (
        (!detail.is_noidentity || item.nik !== '') &&
        (!detail.is_name || item.full_name !== '') &&
        (!detail.is_email || item.email !== '') &&
        (!detail.is_phone_number || item.no_telp !== '')
      );
    });
  ...
  const copyOrderer = (targetIndex: number) => {
    if (form.length > 0 && targetIndex > 0 && targetIndex < form.length) {
      ...
      setForm(newForm);
      const isFormValid = newForm.every(
        (item) =>
        ...
        <div className='border-b p-3 border-primary-light flex items-center gap-3'>
          ...
          <div className='border border-primary-light-200 rounded-lg bg-white shadow-sm'>
                 ...
                 <div className='py-3 px-4 flex justify-between items-center'>
                  ...
                  <div className='py-3 px-4 flex justify-between items-center'>
                    <p>Biaya Admin</p>
                    <p className='font-semibold'>Rp{detail.admin_fee.toLocaleString('id-ID')}</p>
                  </div>
                  {detail.ppn ? (
                    <div className='py-3 px-4 flex justify-between items-center'>
                      <p>Tax</p>
                      ...
                      {detail.is_noidentity ? (
                        <Field className='mb-2'>
                          ...
                          {error.nik && item.nik.length < 16 && (
                            <p className="text-[10px] mt-1 text-danger">Minimal NIK adalah 16 Digit</p>
                          )}
                          {error.nik && item.nik.length > 17 && (
                            <p className="text-[10px] mt-1 text-danger">Maksimal NIK adalah 17 Digit</p>
                          )}
                        </Field>
                      ) : null}
                      {detail.is_name ? (
