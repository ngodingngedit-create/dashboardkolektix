...
const FirstStep = ({
  ...
  const handleGetVoucher = async (index: number) => {
    ...
    >({
      ...
      success: (data) => {
        ...
        if (!voucher) return;
        const isDateValid =
          moment(voucher.date_start).isBefore(new Date()) &&
          moment(voucher.date_end).isAfter(new Date());
        const isStockValid = voucher.stock > 0;
        const isStatusValid = voucher.status == 1;
        const isMinTransactionValid =
          totalSubtotalPrice >= voucher.min_transaction;
  ...
  useEffect(() => {
    if (Array.isArray(haveVoucher) && haveVoucher.length > 0) {
      setVoucherFields(
        haveVoucher.map((voucher: { name: string }) => voucher.name || "")
      );
      setVouchers((prev) => {
      ...
      <div className="bg-primary-light px-4 sm:px-8 md:px-12 lg:px-0 mb-72">
              ...
              <Group key={index}>
                ...
                {vouchers[index] && (
                  <>
                    ...
                    <Icon
                      icon="uiw:circle-check"
                      className="text-green-500 text-[20px] shrink-0"
                    />
                  </>
                )}
              </Group>
        ...
        {form.map((item, index) => {
          ...
          for (const ticketItem of ticket) {
            for (
              let i = 0;
              i < (ticketItem?.seat_number?.length ?? ticketItem.qty_ticket);
              i++
            ) {
              if (currentIndex === index - 1) {
                ...
                ticketForOwner = {
                  ...ticketItem,
                  seat_number: ticketItem?.seat_number
                    ? ticketItem?.seat_number[i]
                    : undefined,
                } as FormTicket;
                break;
              }
          ...
          return (
            <div className="bg-white mt-4 last:mb-16" key={index}>
              <div
                className="border-b py-3 px-5 border-primary-light flex items-center justify-between cursor-pointer"
                onClick={() => toggleCollapse(index)}
              >
                {index > 0 && (
                  <FontAwesomeIcon
                    icon={faTicket}
                    className="text-primary shrink-0 mr-[10px]"
                  />
                )}
                  ...
                  <p className="font-semibold">
                    {index > 0
                      ? `${index}. ${t("ticketOwner")} ${
                          ...
                          ticketForOwner?.seat_number
                            ? `(Seat ${ticketForOwner?.seat_number})`
                            : ""
                        }`
                      : t("registrantData")}
                  </p>
              ...
              <div
                className={`border-b p-3 border-primary-light ${
                  collapse[index] ? "max-h-[50rem]" : "max-h-0"
              ...
              >
                ...
                >
                  <div
                    className={`${
                      collapse[index]
                        ? "visible delay-300 duration-300"
                        : "invisible"
                    } transition-transform `}
                  >
                    {detail.is_noidentity ? (
                      <Field className="mb-2">
                        <Label className="text-sm font-base text-grey">
                          Nomor Induk KTP
                        </Label>
                        <Input
                          type="text"
                          ...
                          maxLength={17}
                        />
                        {error.nik && item.nik.length < 16 && (
                          <p className="text-[10px] mt-1 text-danger">
                            Minimal NIK adalah 16 Digit
                          </p>
                        )}
                        {error.nik && item.nik.length > 17 && (
                          <p className="text-[10px] mt-1 text-danger">
                            Maksimal NIK adalah 17 Digit
                          </p>
                        )}
                      </Field>
                    ) : null}
                    {detail.is_name ? (
                      <Field className="mb-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInput(index, "full_name", e.target.value)
                          }
                        />
                      </Field>
                    ) : null}
                    {detail.is_profession ? (
                      <Field className="mb-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInput(index, "is_profession", e.target.value)
                          }
                        />
                      </Field>
                    ) : null}
                    {detail.is_company ? (
                      <Field className="mb-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInput(index, "is_company", e.target.value)
                          }
                        />
                      </Field>
                    ) : null}
                      ...
                      <Field className="mb-2">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <Input
                            ...
                            value={item.no_telp}
                            onChange={(e) =>
                              handleInput(index, "no_telp", e.target.value)
                            }
                          />
                        </div>
                      </Field>
            ...
            {form.map((item, index) => {
              ...
              for (const ticketItem of ticket) {
                for (
                  let i = 0;
                  i <
                  (ticketItem?.seat_number?.length ?? ticketItem.qty_ticket);
                  i++
                ) {
                  if (currentIndex === index - 1) {
                    // Pemilik ditemukan, simpan tiket untuk pemilik ini
              ...
              return (
                <div
                  className="border border-primary-light-200 rounded-lg bg-white shadow-sm"
                  key={index}
                >
                  <div
                    className="border-b border-b-primary-light-200 cursor-pointer px-5 py-3 flex items-center justify-between"
                    onClick={() => toggleCollapse(index)}
                  >
                    {index > 0 && (
                      <FontAwesomeIcon
                  ...
                  >
                    ...
                    >
                      <div
                        className={`${
                          collapse[index] ? "visible" : "invisible"
                        } flex flex-col gap-3`}
                      >
                        {detail.is_noidentity ? (
                          <>
                            <InputField
                              ...
                              type="number"
                              label={t("ktpNumber")}
                              placeholder={`${t("example")}: 123456789012345`}
                              value={item.nik}
                              onChange={(e) =>
                                handleInput(index, "nik", e.target.value)
                              }
                            />
                            {error.nik && (
                              <p className="text-[10px] mt-1 text-danger">
                        ...
                        {detail.is_phone_number ? (
                          <>
                            <InputField
                              ...
                              type="number"
                              label={t("phoneNumber")}
                              placeholder={`${t("example")}: 81233334444`}
                              onChange={(e) =>
                                handleInput(index, "no_telp", e.target.value)
                              }
                              value={item.no_telp}
              ...
              <div className="py-3 px-4 flex justify-between items-center">
                ...
                <p className="font-semibold">
                  {(() => {
                    ...
                    const tax = detail.ppn
                      ? Math.round(
                          (subtotal + totalTicketFee) * (detail.ppn / 100)
                        )
                      : 0;
                    const grandTotal =
                      subtotal + totalTicketFee + tax - voucherDiscount;
