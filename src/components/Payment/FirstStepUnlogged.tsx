        ...
        await fetch<{
            ...
            success: (data) => {
                ...
                if (isDateValid && isStockValid && isStatusValid && isMinTransactionValid) {
                    ...
                    setVouchers((prevVouchers) => [...prevVouchers, {data: voucher, id: voucher.id, name: voucherFields[index], amount: discount}]); 
                } else {
                    notifications.show({
                                                    ...
                                                    <Field className="mb-2">
                                                        ...
                                                        <div className="flex gap-2 items-center">
                                                            <form className="max-w-sm block mt-2">
                                                                <select id="countries" className="bg-gray-50 border border-primary-light-200 text-dark text-sm rounded-lg focus:ring-primary-base focus:border-primary-light-200 block w-full py-1.5" defaultValue="+62" value={item.countryCode} onChange={(e) => handleInput(index, 'countryCode', e.target.value)}>
                    ...
                    <div className="bg-white mt-1">
                        <div className="border-b-2 p-3 border-primary-light flex flex-col gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-sm mb-1">{xenditInvoice ? `Rp${xenditInvoice.transfer_amount.toLocaleString('id-ID')}` : `Rp${(transactionData.grandtotal - vouchers.reduce((sum, v) => sum + (v?.amount || 0), 0)).toLocaleString('id-ID')}`}</p>
                            </div>
                        </div>
                    </div>
                </div>
