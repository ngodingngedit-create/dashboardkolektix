...
const FirstStepUnlogged = ({
  ...
  const handleInput = (index: number, field: keyof Form, value: string) => {
    ...
    const ticketPriceTotal = ticket.reduce(
      (e, n) => e + n.price * n.qty_ticket,
      0
    );
  ...
  useEffect(() => {
    if (detail && detail.has_event_payment_method.length === 1) {
      ...
      const paymentMethod =
        detail.has_event_payment_method[0].has_payment_method;
  ...
  const submitForm = () => {
    ...
    const allSeatNumbers = ticket
      .map((t) =>
        Array.isArray(t.seat_number)
          ? t.seat_number
          : JSON.parse(t.seat_number || "[]")
      )
      .flat();
    ...
    Post("transaction-without-auth", payload)
      .then((res: any) => {
        ...
        if (res.xendit_invoice && res.xendit_invoice.invoice_url) {
          ...
          res.xendit_invoice &&
          res.xendit_invoice.va_number?.length > 0
        ) {
              ...
              <Stack gap={20}>
                ...
                {voucherFields.map((field, index) => (
                  <Group key={index}>
                    ...
                    />
                    <Button
                      loading={loadings.includes(`getvoucher-${index}`)}
                ...
                <div className="bg-white mt-1" key={index}>
                  ...
                  >
                    ...
                    <Stack gap={0} className={`flex-grow`}>
                      <p className="font-semibold">
                        {index > 0
                          ? `${index}. Pemilik Tiket ${ticketForOwner?.name} ${
                        ...
                        >
                          ...
                          el.has_payment_method.has_payment_link.length > 0 ? (
                            ...
                            <RadioGroup
                              ...
                              name="payment-method"
                              onChange={(e) =>
                                handlePaymentChange(e.target.value, "payment")
                              }
                            >
                ...
                {form.map((item, index) => {
                  ...
                  for (const ticketItem of ticket) {
                    for (
                      let i = 0;
                      i <
                      (ticketItem?.seat_number?.length ??
                        ticketItem.qty_ticket);
                      i++
                    ) {
                      if (currentIndex === index - 1) {
                        // Pemilik ditemukan, simpan tiket untuk pemilik ini
                  ...
                  <div className="py-3 px-4 flex justify-between items-center">
                    ...
                    <p className="font-semibold">
                      {(() => {
                        ...
                        const tax = detail.ppn
                          ? Math.round(
                              (subtotal + adminFee) * (detail.ppn / 100)
                            )
                          : 0;
                        const grandTotal =
                          subtotal + adminFee + tax - voucherDiscount;
            ...
            <div className="border-b-2 p-3 border-primary-light flex flex-col gap-2">
              ...
              <div>
                ...
                <p className="text-sm mb-1">
                  {xenditInvoice
                    ? `Rp${xenditInvoice.transfer_amount.toLocaleString(
                        "id-ID"
                      )}`
                    : `Rp${(
                        transactionData.grandtotal -
                        vouchers.reduce((sum, v) => sum + (v?.amount || 0), 0)
